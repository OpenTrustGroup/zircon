...
#if ENABLE_SMC_TEST
static void* map_shm(zx_info_ns_shm_t* shm_info) {
    ...
    zx_status_t status = VmAspace::kernel_aspace()->AllocPhysical(
            ...
            static_cast<paddr_t>(shm_info->base_phys), VmAspace::VMM_FLAG_COMMIT,
...
static long write_shm() {
    bool is_fail = false;
    zx_info_ns_shm_t shm_info = smc_disp->GetShmInfo();
...
zx_status_t SmcDispatcher::Create(uint32_t options, fbl::RefPtr<SmcDispatcher>* dispatcher,
    ...
    if (smc_disp == nullptr) {
        ...
        fbl::AllocChecker ac;
        auto disp = new (&ac) SmcDispatcher(options, shm_info);
        if (!ac.check()) {
...
SmcDispatcher::SmcDispatcher(uint32_t options, zx_info_ns_shm_t info)
...
zx_info_ns_shm_t SmcDispatcher::GetShmInfo() {
    return shm_info_;
}
