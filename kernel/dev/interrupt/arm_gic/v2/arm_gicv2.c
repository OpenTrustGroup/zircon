...
#if !WITH_LIB_SM
    /* Due to Linux Android custom IPI driver will trigger an IPI when an IPI
     * number is enabled, we cannot register the halt IPI in order to prevent
     * secure OS from getting into halt state after halt IPI is registered in Linux.
     */
