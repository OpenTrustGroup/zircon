...
static zx_status_t trusty_ioctl(void* ctx, uint32_t op, const void* cmd, size_t cmdlen,
    ...
    zxlogf(ERROR, "trusty ioctl");
    auto vdev = reinterpret_cast<TrustyVirtioDevice*>(ctx);
    ...
    case IOCTL_TRUSTY_VDEV_START: {
        ...
        if (vdev->msg_channel() != ZX_HANDLE_INVALID) {
        ...
        status = vdev->Start(fbl::move(ch1));
        if (status != ZX_OK) {
        ...
        zx_handle_t out = ch0.get();
        memcpy(reply, &out, sizeof(zx_handle_t));
