...
// TODO(sy): bus_device will be overrided to 0 if we don't lower optimization level.
// After investigation, it looks like a compiler bug. Let's revisit it after next
// compiler upgradation.
extern "C" zx_status_t __attribute__((optimize("O1"))) virtio_trusty_bind(void* ctx, zx_device_t* bus_device) {
    ...
    fbl::Vector<fbl::unique_ptr<trusty_virtio::Device>> devices;
    ...
    for (auto& entry : entries) {
        ...
        case RSC_VDEV: {
            ...
            TRACEF("Probed trusty vdev '%s'\n", descr->config.dev_name);

            fbl::unique_ptr<trusty_virtio::Backend> backend =
