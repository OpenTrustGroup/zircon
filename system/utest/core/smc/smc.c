...
static bool setup(void) {
    ...
    ASSERT_EQ(zx_smc_create(0, &smc_info, sizeof(zx_info_smc_t), &smc_handle, &shm_vmo_handle),
...
static bool smc_create_multiple_test(void) {
    ...
    ASSERT_EQ(zx_smc_create(0, &tmp_smc_info, sizeof(zx_info_smc_t), &h1, &h2),
...
static void* wait_smc_call(void* args) {
    ...
    zx_status_t status = zx_smc_wait_for_request(smc_handle, smc_args, sizeof(smc32_args_t));
    if (status == ZX_OK) {
        zx_smc_set_result(smc_handle, 0);
    }
...
static bool smc_handle_request_test(void) {
    ...
    ASSERT_EQ(zx_smc_call_test(smc_handle, &expect_smc_args, sizeof(smc32_args_t), &smc_ret),
