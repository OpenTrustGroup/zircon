...
static bool setup(TestContext* ctx) {
    BEGIN_HELPER;
    zx_info_ns_shm_t* shm_info = &ctx->shm_info;

    ASSERT_EQ(zx::smc::create(0, &ctx->smc),
              ZX_OK, "failed to create smc object");

    ASSERT_EQ(zx::resource::create_ns_mem(0, shm_info, &ctx->shm_rsc),
              ZX_OK, "failed to create shm resource");

    ASSERT_EQ(zx::vmo::create_ns_mem(ctx->shm_rsc, shm_info->base_phys,
                                     shm_info->size, &ctx->shm_vmo),
...
static bool smc_create_test(void) {
    ...
    auto shm_info = ctx.shm_info;
    EXPECT_GT(shm_info.base_phys, (uint32_t)0, "ns-shm pa should not be zero");
    EXPECT_GT(shm_info.size, (uint32_t)0, "ns-shm size should not be zero");
    EXPECT_EQ(shm_info.base_phys % PAGE_SIZE, (uint32_t)0, "ns-shm pa should be page aligned");
    EXPECT_EQ(shm_info.size % PAGE_SIZE, (uint32_t)0, "ns-shm size should be page aligned");
    EXPECT_EQ(shm_info.use_cache, (bool)true, "default ns-shm cache policy is enabled");
...
static void* wait_smc_call(void* args) {
    ...
    zx_status_t status = ctx->smc.wait_one(ZX_SMC_READABLE, zx::time::infinite(), NULL);
    if (status != ZX_OK)
        return NULL;

    status = ctx->smc.read(&ctx->smc_args);
    if (status == ZX_OK) {
...
static bool smc_handle_request_good_path_test(void) {
    ...
    EXPECT_EQ(ctx.smc_args.smc_nr, expect_smc_args.smc_nr, "wrong smc_nr");
    EXPECT_EQ(ctx.smc_args.params[0], expect_smc_args.params[0], "wrong param[0]");
    EXPECT_EQ(ctx.smc_args.params[1], expect_smc_args.params[1], "wrong param[1]");
    EXPECT_EQ(ctx.smc_args.params[2], expect_smc_args.params[2], "wrong param[2]");
...
static bool smc_handle_request_bad_path_test(void) {
    ...
    ASSERT_EQ(ctx.smc.wait_one(ZX_SMC_READABLE, zx::time::infinite(), NULL),
              ZX_OK, "wait smc");

    smc32_args_t smc_args;
    ASSERT_EQ(ctx.smc.read(&smc_args), ZX_OK, "smc_read");
    EXPECT_EQ(smc_args.smc_nr, (uint32_t)SMC_SC_VIRTIO_START, "wrong smc_nr");

    /* read twice should return error */
    ASSERT_EQ(ctx.smc.read(&smc_args), ZX_ERR_SHOULD_WAIT, "smc_read");

    ASSERT_EQ(ctx.smc.set_result(SM_OK), ZX_OK, "smc_set_result");

    /* set_result twice should return error */
    ASSERT_EQ(ctx.smc.set_result(SM_OK), ZX_ERR_BAD_STATE, "smc_set_result");
...
static bool smc_shm_vmo_write_test(void) {
    ...
    size_t vmo_size = ctx.shm_info.size;
